-- db/migrations/0031_add_audit_layers.sql

-- L1: 기술 인프라 스냅샷
CREATE TABLE IF NOT EXISTS tech_infra_snapshots (
...
-- L2: 스키마 품질 스냅샷
CREATE TABLE IF NOT EXISTS schema_quality_snapshots (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  workspace_id UUID NOT NULL, -- references workspaces(id)
  website_url TEXT NOT NULL,
  audit_session_id UUID, -- references audit_sessions(id)
...
-- L3: 콘텐츠 시맨틱 스냅샷
CREATE TABLE IF NOT EXISTS content_semantic_snapshots (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  workspace_id UUID NOT NULL, -- references workspaces(id)
  website_url TEXT NOT NULL,
  audit_session_id UUID, -- references audit_sessions(id)
...
-- RLS 활성화 및 권한 설정
ALTER TABLE tech_infra_snapshots ENABLE ROW LEVEL SECURITY;
ALTER TABLE schema_quality_snapshots ENABLE ROW LEVEL SECURITY;
ALTER TABLE content_semantic_snapshots ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Enable all for tech_infra_snapshots" ON tech_infra_snapshots FOR ALL USING (true);
CREATE POLICY "Enable all for schema_quality_snapshots" ON schema_quality_snapshots FOR ALL USING (true);
CREATE POLICY "Enable all for content_semantic_snapshots" ON content_semantic_snapshots FOR ALL USING (true);

-- 인덱스 추가
CREATE INDEX IF NOT EXISTS idx_tech_infra_workspace ON tech_infra_snapshots(workspace_id);
CREATE INDEX IF NOT EXISTS idx_schema_quality_workspace ON schema_quality_snapshots(workspace_id);
CREATE INDEX IF NOT EXISTS idx_content_semantic_workspace ON content_semantic_snapshots(workspace_id);
