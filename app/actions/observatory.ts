...
export async function createResponseJudgment(workspaceId: string, data: any) {
  ...
  const { createResponseJudgmentCore } = await import("../../lib/db/observatory-db");
  return createResponseJudgmentCore(workspaceId, data);
}
...
export async function computeDomainIndexSnapshot(workspaceId: string, definitionId: string | null | undefined, runId: string) {
  ...
  const { computeDomainIndexSnapshotCore } = await import("../../lib/db/observatory-db");
  return computeDomainIndexSnapshotCore(workspaceId, definitionId, runId);
}
