-- supabase/migrations/0012_industry_benchmark.sql
-- 업종별 벤치마크 역설계 시스템
...
-- 개별 사이트 감사 결과 스냅샷
CREATE TABLE IF NOT EXISTS public.benchmark_audit_results (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  reference_site_id UUID REFERENCES public.reference_sites(id) ON DELETE SET NULL,
  sub_industry_key TEXT NOT NULL,
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'audit_sessions' AND column_name = 'relative_position') THEN
    ALTER TABLE public.audit_sessions ADD COLUMN relative_position JSONB;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'audit_sessions' AND column_name = 'improvement_strategy') THEN
    ALTER TABLE public.audit_sessions ADD COLUMN improvement_strategy JSONB;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'audit_sessions' AND column_name = 'industry_blueprint_sub_key') THEN
    ALTER TABLE public.audit_sessions ADD COLUMN industry_blueprint_sub_key TEXT;
  END IF;
END $$;

-- RLS
ALTER TABLE public.reference_sites ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.benchmark_audit_results ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Enable all for reference_sites" ON public.reference_sites FOR ALL USING (true);
CREATE POLICY "Enable all for benchmark_audit_results" ON public.benchmark_audit_results FOR ALL USING (true);
CREATE POLICY "Enable all for industry_benchmark_profiles" ON public.industry_benchmark_profiles FOR ALL USING (true);
CREATE POLICY "Enable all for industry_blueprints" ON public.industry_blueprints FOR ALL USING (true);

-- 인덱스
CREATE INDEX IF NOT EXISTS idx_ref_sites_industry ON public.reference_sites(sub_industry_key);
CREATE INDEX IF NOT EXISTS idx_bench_audit_industry ON public.benchmark_audit_results(sub_industry_key);
CREATE INDEX IF NOT EXISTS idx_bench_audit_audited ON public.benchmark_audit_results(audited_at DESC);
